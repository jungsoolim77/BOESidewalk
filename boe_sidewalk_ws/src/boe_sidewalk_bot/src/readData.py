...
class ReadData:
    ...
    def getDigitalLevel(self):
        self.serialDig.reset_input_buffer()
    ...
    def get_GPS(self):
        self.serialGPS.reset_input_buffer()
        data = (str(self.serialGPS.readline().decode())).split(",")
        if "RMC" in data[0] and data[2]=='A':
            self.get_time(data)
            self.updateGPS(data)
    ...
    def send_data(self):
        while not rospy.is_shutdown():
            ...
            self.raw_data = str(self.lat) + ", " + str(self.lon) + ", " + str(self.x_slope) + ",  " + str(self.y_slope) + ",  " + str(
                self.x_level_temp) + ",  " + str(self.y_level_temp) + ", " + self.gpsData.strip()
            # print(self.raw_data)
